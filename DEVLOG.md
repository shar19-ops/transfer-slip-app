...
## 2026-07-19: 環境依存文字の注意書きに㈱／㈲を追加、支払伝票PJ系アプリ全体で文言統一

### 要件

ユーザー要望「入力フォームの注意書きを『※ 環境依存文字（㈱、㈲、丸囲み数字、ローマ数字、特殊記号など）は
入力しないでください。PDF出力時に正しく表示されない場合があります。』に変更。支払伝票(一般管理費)、
(未成工事用)も同一文章に変更してください」。

### 対応

3アプリの注意書きが実はそれぞれ微妙に異なる文言だったため（振替伝票は「環境依存文字（丸囲み数字…）」、
支払伝票2本は「丸数字・ローマ数字・絵文字などの環境依存文字は文字化け・表示崩れの原因となるため…」）、
ユーザー指定の新文言で3アプリとも完全に統一した:

| リポジトリ | ファイル | コミット |
|---|---|---|
| `transfer-slip-app`（本リポジトリ） | `振替伝票.html` | `f2f34cc` |
| `payment-slip-app` | `支払伝票【未成工事用】.html` | `cf20b5f` |
| `一般管理費支払伝票アプリ_v3`（`shiharai-denpyo`） | `支払伝票【一般管理費】.html` | `88c9549` |

`一般管理費支払伝票アプリ`（v2、`general-affairs-slip-app`）は2026-07-19にv2/v3を分離した際の方針
（v2は今後開発対象外、v3のみ開発継続）により今回は変更していない。
